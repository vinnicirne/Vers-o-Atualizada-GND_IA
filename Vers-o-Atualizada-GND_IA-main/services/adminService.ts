...
export const getGlobalToolSettings = async (): Promise<ToolSetting[]> => {
    // Default: All enabled
    const defaults = CREATOR_SUITE_MODES.map(m => ({ key: m.value, enabled: true }));
    const saved = await getConfig<ToolSetting[]>('global_tools', []);
    
    // Merge saved with defaults to ensure all keys exist
    return defaults.map(d => {
        const s = saved.find(item => item.key === d.key);
        return s ? s : d;
    });
};

export const updateGlobalToolSettings = async (settings: ToolSetting[], adminId: string) => {
    await setConfig('global_tools', settings, adminId);
    logger.info(adminId, 'Sistema', 'update_global_tools', { settings });
};

// --- WHITE LABEL ---
export const getWhiteLabelSettings = async (): Promise<WhiteLabelSettings> => {
    const { data, error } = await api.select('system_config', { key: 'white_label_settings' });
    if (error || !data || data.length === 0) {
        return {} as WhiteLabelSettings; 
    }
    return data[0].value as WhiteLabelSettings;
};
...
export const updateWhiteLabelSettings = async (settings: WhiteLabelSettings, adminId: string) => {
    ...
    logger.info(adminId, 'Sistema', 'update_white_label', { appName: settings.appName });
};