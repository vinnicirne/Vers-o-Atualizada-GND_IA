...
serve(async (req) => {
  ...
  try {
    ...
    if (provider === "mercadopago") {
        ...
        if ((type === "payment" || action === "payment.created" || action === "payment.updated") && data?.id) {
            ...
            if (mpRes.ok) {
                ...
                console.log(`[Webhook MP] ID: ${transactionId}, Status API: ${paymentStatus}, Valor: ${paidAmount}`);
            } else {
    ...
    // --- LÓGICA ASAAS ---
        ...
        console.warn(`[Webhook] Provedor "${provider}" não especificado ou inválido.`);
    ...
    if (transactionId && paymentStatus) {
        console.log(`[Webhook] Processando transação ID: ${transactionId}, Novo Status: ${paymentStatus}`);
        ...
        if (txError && txError.code === 'PGRST116') { // PGRST116 = no rows found
            console.warn(`[Webhook] Transação não encontrada para ID Externo: ${transactionId}. Isso pode ser uma notificação de teste ou um pagamento externo não iniciado pelo nosso app.`);
            // Retorna 200 para o gateway não ficar tentando reenviar se for uma transação que não iniciamos
            return new Response(JSON.stringify({ message: "Transaction not found locally, gracefully ignored" }), { status: 200, headers: corsHeaders });
        } else if (txError) {
            console.error(`[Webhook] Erro ao buscar transação no DB: ${txError.message}`);
            return new Response(JSON.stringify({ error: `Database error: ${txError.message}` }), { status: 500, headers: corsHeaders });
        }

        if (tx) {
            console.log(`[Webhook] Transação local encontrada (ID: ${tx.id}). Status atual: ${tx.status}, Novo status reportado: ${paymentStatus}`);

            // Evita processamento duplicado se já estiver aprovada
            if (tx.status === 'approved' && paymentStatus === 'approved') {
                 console.log(`[Webhook] Transação ${tx.id} já está aprovada. Ignorando atualização.`);
            ...
            else if (paymentStatus === 'rejected' || paymentStatus === 'cancelled' || paymentStatus === 'failed' || paymentStatus === 'CHARGED_BACK' || paymentStatus === 'OVERDUE') newDbStatus = 'failed';
            ...
            console.log(`[Webhook] Atualizando status DB de ${tx.status} para ${newDbStatus} para tx ID: ${tx.id}`);
            ...
            if (newDbStatus === 'approved') {
                console.log(`[Webhook] Status ${newDbStatus} detectado. Iniciando liberação de benefícios para usuário ${tx.usuario_id}.`);
                ...
                if (userError) console.warn(`[Webhook] Erro ao buscar dados do usuário ${tx.usuario_id}: ${userError.message}`);
    ...
    console.log(`[Webhook] Processamento finalizado. Retornando 200 OK.`);
    ...
    console.error("Webhook Critical Error during execution:", err);
...
async function releaseBenefits(supabaseAdmin: any, userId: string, itemType: string, itemId: string, amount: number, referrerId?: string) {
    console.log(`[Benefits Helper] Iniciando liberação para userId: ${userId}, itemType: ${itemType}, itemId: ${itemId}, amount: ${amount}, referrerId: ${referrerId}`);
    
    // Libera plano ou créditos
    if (itemType === "plan") {
        console.log(`[Benefits Helper] Atualizando plano para ${itemId} para o usuário ${userId}`);
        ...
        if (configError) console.error(`[Benefits Helper] Erro ao buscar config de planos: ${configError.message}`);

        if (config?.value) {
          const plan = (config.value as any[]).find((p: any) => p.id === itemId);
          if (plan) {
            console.log(`[Benefits Helper] Atribuindo ${plan.credits} créditos do plano ${itemId} para ${userId}`);
            await supabaseAdmin.from("user_credits").upsert({ user_id: userId, credits: plan.credits }, { onConflict: "user_id" });
          } else {
            console.warn(`[Benefits Helper] Plano "${itemId}" não encontrado na configuração de sistema. Créditos não atribuídos.`);
          }
        } else {
            console.warn(`[Benefits Helper] Configuração 'all_plans' não encontrada ou vazia.`);
        }
    } else if (itemType === "credits") {
        console.log(`[Benefits Helper] Adicionando ${itemId} créditos para o usuário ${userId}`);
        ...
        if (currentCreditsError) console.warn(`[Benefits Helper] Erro ao buscar créditos atuais para ${userId}: ${currentCreditsError.message}`);
        ...
        if (currentCredits !== -1) {
            ...
            console.log(`[Benefits Helper] Créditos de ${currentCredits} para ${newCredits} para ${userId}`);
            await supabaseAdmin.from("user_credits").upsert({ user_id: userId, credits: newCredits }, { onConflict: "user_id" });
        } else {
            console.log(`[Benefits Helper] Usuário ${userId} tem créditos ilimitados. Nenhuma alteração de saldo.`);
        }
    }

    // Processa Comissão de Afiliado (20%)
    if (referrerId) {
        console.log(`[Benefits Helper] Processando comissão de afiliado para ${referrerId} da compra de ${userId}`);
        ...
        if (commission > 0) {
            ...
            if (refUserError) console.error(`[Benefits Helper] Erro ao buscar afiliado ${referrerId}: ${refUserError.message}`);

            if (refUser) {
                const newBalance = (refUser.affiliate_balance || 0) + commission;
                console.log(`[Benefits Helper] Atualizando saldo do afiliado ${referrerId} de ${refUser.affiliate_balance} para ${newBalance}. Comissão: ${commission}`);
            ...
            console.log(`[Benefits Helper] Comissão calculada foi zero ou negativa (${commission}). Não será processada.`);
        }
    } else {
        console.log(`[Benefits Helper] Usuário ${userId} não foi indicado. Nenhuma comissão de afiliado.`);
    }
    console.log(`[Benefits Helper] Liberação de benefícios concluída.`);
}