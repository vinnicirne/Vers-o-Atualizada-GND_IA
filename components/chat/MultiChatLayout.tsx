...
export function MultiChatLayout({ user }: MultiChatLayoutProps) {
    ...
    const handleManageAgents = () => {
        alert("Funcionalidade de Gerenciar Agentes em desenvolvimento. Em breve você poderá convidar membros da equipe para este número.");
    };

    return (
        <div className="flex h-[calc(100vh-130px)] bg-gray-100 overflow-hidden font-sans border-t border-gray-200">
            {/* 1. LEFT SIDEBAR: INSTANCES & SETTINGS */}
            <div className="w-16 bg-[#202C33] flex flex-col items-center py-4 gap-4 z-20 shadow-lg">
                {instances.map(inst => (
                    <button 
                        key={inst.id}
                        onClick={() => setSelectedInstance(inst.id)}
                        className={`w-10 h-10 rounded-full flex items-center justify-center transition relative group ${selectedInstance === inst.id ? 'bg-[#00A884] text-white ring-2 ring-white/50' : 'bg-gray-700 text-gray-400 hover:bg-gray-600'}`}
                        title={inst.name}
                    >
                ...
                <button 
                    ...
                    title={`Adicionar Número (${instances.length}/${maxInstances})`}
                >
                ...
                <div className="mt-auto flex flex-col items-center gap-3">
                    {selectedInstance && (
                        <button 
                            ...
                            className="w-10 h-10 rounded-full bg-blue-600/20 text-blue-400 hover:bg-blue-600 hover:text-white flex items-center justify-center transition" 
                            title="Gerenciar Agentes"
                        >
                            <i className="fas fa-users-cog"></i>
                        </button>
                    )}
                    <button className="text-gray-400 hover:text-white p-2" title="Configurações"><i className="fas fa-cog"></i></button>
                </div>
            </div>
            ...
            <div className="w-80 bg-white border-r border-gray-200 flex flex-col z-10">
                <div className="h-16 bg-[#F0F2F5] px-4 flex items-center justify-between border-b border-gray-300">
                    <h2 className="font-bold text-[#263238] flex items-center gap-2">
                        Conversas
                        <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{conversations.length}</span>
                    </h2>
                    <div className="flex gap-2">
                        <button className="text-gray-500 hover:bg-gray-200 p-2 rounded-full transition"><i className="fas fa-comment-medical"></i></button>
                        <button className="text-gray-500 hover:bg-gray-200 p-2 rounded-full transition"><i className="fas fa-ellipsis-v"></i></button>
                    </div>
                </div>
                
                {/* Search */}
                <div className="p-2 bg-white border-b border-gray-100">
                    <div className="bg-[#F0F2F5] rounded-lg flex items-center px-3 py-1.5 border border-transparent focus-within:border-green-500 transition-colors">
            ...
            <div className="flex-1 relative flex flex-col">
                {selectedChat ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center bg-[#F0F2F5] border-b-[6px] border-green-500 text-center p-8">
                        <div className="w-64 h-64 bg-[url('https://cdn-icons-png.flaticon.com/512/1077/1077063.png')] bg-contain bg-no-repeat bg-center opacity-10 mb-6 animate-pulse"></div>
                        ...
                        <p className="text-sm text-gray-500 max-w-md">
                            ...
                            Use o WhatsApp em até <strong>{maxInstances} números</strong> ao mesmo tempo.
                        </p>
                        <div className="mt-8 flex items-center gap-2 text-xs text-gray-400 bg-gray-200 px-3 py-1 rounded-full">
