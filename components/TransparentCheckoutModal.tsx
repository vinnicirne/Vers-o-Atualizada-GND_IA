...
export function TransparentCheckoutModal({
  ...
  const [formData, setFormData] = useState({
    ...
    cardholderEmail: payerEmail, // Use payerEmail as default for cardholderEmail
  });
  const [docTypes, setDocTypes] = useState<{ id: string; name: string }[]>([]);
  // const [paymentMethods, setPaymentMethods] = useState<{ id: string; name: string; thumbnail: string; min_allowed_amount: number; max_allowed_amount: number; }[]>([]); // Not strictly needed, onPaymentMethodReceived handles it
  ...
  useEffect(() => {
    if (!isOpen) {
        // Cleanup on close to ensure a fresh start next time
        ...
        setFormData(prev => ({ ...prev, cardholderEmail: payerEmail, cardNumber: '', cardholderName: '', cardExpirationMonth: '', cardExpirationYear: '', securityCode: '', issuer: '', installments: '1', identificationNumber: '' })); // Reset all form fields, keep default docType
        return;
    }
    ...
    const cardFormInstance = mp.cardForm({
      ...
      callbacks: {
        ...
        onFormMounted: (error: any) => {
          if (error) {
            console.error("Mercado Pago form mounted with error:", error);
            setError(error.message || "Erro ao montar formulário do Mercado Pago.");
            return;
          }
          // Reset form data to clear any previous state not covered by unmount
          // and ensure cardholderEmail is set from payerEmail
          setFormData(prev => ({ 
            ...prev, 
            cardholderEmail: payerEmail,
            cardNumber: '', cardholderName: '', cardExpirationMonth: '', cardExpirationYear: '', securityCode: '',
            issuer: '', installments: '1', identificationNumber: ''
          }));
        },
        onFormUnmounted: (error: any) => {
            if (error) {
                console.error("Mercado Pago form unmounted with error:", error);
                return;
            }
        },
        ...
        onCardTokenReceived: (token: MercadoPagoCardToken, mappedCallbacks: any) => {
          ...
          if (!formData.cardholderEmail || !formData.cardholderEmail.trim()) {
          ...
          const paymentData: TransparentPaymentFormData = {
            token: token.id,
            payerEmail: formData.cardholderEmail, // Use the email from form state
            ...
            metadata: {
              ...metadata,
              card_issuer_id: formData.issuer, // Add issuer ID to metadata
        ...
        onInstallmentsReceived: (installments: any[]) => {
            ...
            // Ensure installments[0] exists before accessing payer_costs
            setInstallmentsOptions(installments?.[0]?.payer_costs || []);
            // Set default installment to 1 if options are available and current is not valid
            if (installments?.[0]?.payer_costs?.length > 0) {
                const defaultInstallment = installments[0].payer_costs.find((opt:any) => opt.quantity === 1) || installments[0].payer_costs[0];
                setFormData(prev => ({ ...prev, installments: String(defaultInstallment.quantity) }));
            } else {
        ...
        onPaymentMethodReceived: (paymentMethod: any) => {
            ...
            setIssuers([]); 
            setFormData(prev => ({ ...prev, issuer: '' })); // Clear issuer selection
        ...
        onError: (err: any) => {
            ...
            let errorMessage = 'Ocorreu um erro ao processar o pagamento. Verifique os dados do cartão.';
            if (Array.isArray(err) && err.length > 0) {
                errorMessage = err.map((e: any) => e.message || `Erro de validação: ${e.code || JSON.stringify(e)}`).join('; ');
            } else if (err && typeof err.message === 'string') {
                errorMessage = err.message;
            } else if (err) {
                errorMessage = `Erro desconhecido do Mercado Pago: ${JSON.stringify(err)}`;
            }
  ...
  }, [isOpen, amount, description, payerEmail, publicKey, metadata, onSubmit]); // Removed formData from dependencies to prevent infinite loops, onFormMounted now handles reset.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Basic client-side validation for required fields before attempting to create token
    const requiredFields = ['cardNumber', 'cardholderName', 'cardExpirationMonth', 'cardExpirationYear', 'securityCode', 'identificationNumber', 'cardholderEmail'];
    for (const field of requiredFields) {
      if (!formData[field as keyof typeof formData].trim()) {
        const fieldNameMap: Record<string, string> = {
            cardNumber: 'número do cartão',
            cardholderName: 'nome no cartão',
            cardExpirationMonth: 'mês de vencimento',
            cardExpirationYear: 'ano de vencimento',
            securityCode: 'código de segurança',
            identificationNumber: 'número do documento',
            cardholderEmail: 'e-mail do titular',
        };
        setError(`Campo obrigatório: ${fieldNameMap[field] || field}.`);
        return;
      }
    }
          ...
          <form id="form-checkout" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  maxLength={4} // Some cards have 3, some 4. Max 4 covers both.
                  className={inputClasses}
                  disabled={isProcessing}
                  required
                  autoComplete="cc-csc"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  disabled={isProcessing}
                  required
                  autoComplete="off" // No standard autocomplete for this
                />
              </div>
            </div>

            {/* Issuer (Banco Emissor) - Always render */}
            <div>
                ...
                <select
                    ...
                    onChange={handleChange}
                    className={inputClasses + " cursor-pointer"}
                    disabled={isProcessing || issuers.length === 0} // Disable if no issuers are loaded
                    required
                >
                    {issuers.length === 0 && <option value="">Carregando...</option>}
                    {issuers.map(issuer => (
            ...
            <div>
                ...
                <select
                    ...
                    onChange={handleChange}
                    className={inputClasses + " cursor-pointer"}
                    disabled={isProcessing || installmentsOptions.length === 0} // Disable if no options are loaded
                    required
                >
                    {installmentsOptions.length === 0 && <option value="1">Carregando...</option>}
            ...
            <div>
              ...
              <input
                type="email"
                id="cardholderEmail"
                name="cardholderEmail" // Changed name to match formData field
