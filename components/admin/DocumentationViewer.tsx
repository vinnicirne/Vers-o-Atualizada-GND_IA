...
-- Campos para Assinatura Recorrente (Asaas)
...
-- 4. TABELA DE MEMÓRIA DO USUÁRIO
...
-- 5. TABELA DE DOMÍNIOS PERMITIDOS
    ...
    "nodes": [
      ...
      },
      {
        "parameters": {
          "conditions": {
            "string": [
              { "value1": "={{ $json.body.prompt }}", "operation": "isNotEmpty" },
              { "value1": "={{ $json.params.userId }}", "operation": "isNotEmpty" }
            ]
          }
        },
        "id": "validate-input",
      ...
      },
      {
        "parameters": {
          ...
          "bodyParameters": {
            "parameters": [
              { "name": "contents", "value": "={{ [{'parts': [{'text': $json.body.prompt }]}] }}" }
            ]
          },
          "options": {
      ...
      },
      {
        "parameters": {
          "conditions": {
            "boolean": [
              { "value1": "={{ $json.error === undefined }}", "value2": true }
            ]
          }
        },
    ...
    "connections": {
      ...
      "Validar Entrada": {
        "main": [
          ...
          [ { "node": "Erro Validação (400)", "type": "main", "index": 0 } ]
        ]
      },
      "Gemini AI": {
        "main": [
          [ { "node": "Verificar Sucesso API", "type": "main", "index": 0 } ]
        ]
      },
      "Verificar Sucesso API": {
        "main": [
          [ { "node": "Resposta Sucesso", "type": "main", "index": 0 } ],
          [ { "node": "Erro API (500)", "type": "main", "index": 0 } ]
        ]
      }
    }
  }, null, 2);
      ...
      <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-200">
        ...
        {activeTab === 'updates' && (
            <div className="prose prose-slate max-w-none">
                ...
                <p className="text-sm text-gray-500 mb-4">Execute este script no SQL Editor do Supabase para atualizar o banco de dados com todas as tabelas necessárias.</p>
