...
export default function CheckoutCompleto({
  ...
  const handleMpPixGenerate = async () => {
    ...
    try {
      ...
      setPixData({ qrCode: qrData.qr_code_base64, copyPaste: qrData.qr_code, id: result.id.toString() });
      onSuccess();
    } catch (err: any) {
      setError(err.message);
      onError(err.message);
  ...
  return (
    <div className="relative max-w-md mx-auto bg-gray-950 p-6 rounded-2xl border border-gray-800 shadow-2xl">
      <div className="flex justify-between mb-6">
        <h3 className="text-lg font-bold text-white"><i className="fas fa-lock text-green-500"></i>